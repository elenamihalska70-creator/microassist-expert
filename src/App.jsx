...
const currentPlanLimits = PRICING_LIMITS[userPlan] || PRICING_LIMITS.free;
  ...
  const currentPlan = fiscalProfile?.plan || userPlan || "free";
...
useEffect(() => {
 ...
 function handleOpenRevenuePopup() {
  if (revenues.length >= currentPlanLimits.revenues) {
    handleOpenSaveModal("revenue_limit");
    return;
  }
...
function handleOpenInvoiceGenerator() {
  if (invoicesThisMonth >= currentPlanLimits.invoicesPerMonth) {
    handleOpenSaveModal("invoice_limit");
    return;
  }
...
function handleExportPDFWithLimit() {
  if (pdfExportCount >= currentPlanLimits.pdfExportsPerMonth) {
    handleOpenSaveModal("pdf_export_limit");
    return;
  }
          ...
          {appView === "dashboard" && (
            <section ref={fiscalRef} className="card">
              <div className="sectionHead">
                ...
                {currentPlan === "beta_founder" && !isTrialExpired && (
  <div
    style={{
      marginTop: 10,
      display: "inline-flex",
